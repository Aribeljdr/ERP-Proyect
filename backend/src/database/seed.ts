...
async function seed() {
  ...
  await em.query('TRUNCATE TABLE users, clients, kpis, activities, meetings, kanban_cards, sales_orders, invoices, purchase_orders, products, tickets, campaigns, email_templates, landing_pages, employees, payroll_records, attendance_records, expenses, treasury_movements, journal_entries, documents RESTART IDENTITY CASCADE');
  ...
  await em.save(Kpi, kpiData.map(d => em.create(Kpi, d)));

  const activityData = [
    { who: 'Laura M.', what: 'creó la factura #INV-2041', when: 'Hace 5 min', icon: 'invoice', color: '#2563EB', tint: 'rgba(37,99,235,.1)' },
    { who: 'Diego R.', what: 'cerró el trato con Nexus Ltd.', when: 'Hace 22 min', icon: 'check', color: '#22C55E', tint: 'rgba(34,197,94,.1)' },
    { who: 'Sistema', what: 'stock bajo en "Teclado Pro"', when: 'Hace 1 h', icon: 'inventory', color: '#F59E0B', tint: 'rgba(245,158,11,.1)' },
    { who: 'Ana P.', what: 'agregó 3 nuevos leads', when: 'Hace 2 h', icon: 'users', color: '#4F46E5', tint: 'rgba(79,70,229,.1)' },
    { who: 'Carlos V.', what: 'respondió al ticket #882', when: 'Hace 3 h', icon: 'tickets', color: '#0EA5E9', tint: 'rgba(14,165,233,.1)' },
  ];
  await em.save(Activity, activityData.map(d => em.create(Activity, d)));
  ...
  await em.save(Meeting, meetingData.map(d => em.create(Meeting, d)));
  ...
  await em.save(Client, clientData.map(d => em.create(Client, d)));
  ...
  await em.save(KanbanCard, cardData.map(d => em.create(KanbanCard, d)));
  ...
  await em.save(SalesOrder, salesData.map(d => em.create(SalesOrder, d)));
  ...
  await em.save(Invoice, invoiceData.map(d => em.create(Invoice, d)));
  ...
  await em.save(PurchaseOrder, purchaseData.map(d => em.create(PurchaseOrder, d)));
  ...
  await em.save(Product, productData.map(d => em.create(Product, d)));
  ...
  await em.save(Ticket, ticketData.map(d => em.create(Ticket, d)));
  ...
  await em.save(Campaign, campaignData.map(d => em.create(Campaign, d)));
  ...
  await em.save(EmailTemplate, emailTemplateData.map(d => em.create(EmailTemplate, d)));
  ...
  await em.save(LandingPage, landingPageData.map(d => em.create(LandingPage, d)));
  ...
  await em.save(Employee, employeeData.map(d => em.create(Employee, d)));
  ...
  await em.save(Payroll, payrollData.map(d => em.create(Payroll, d)));
  ...
  await em.save(AttendanceRecord, attendanceData.map(d => em.create(AttendanceRecord, d)));
  ...
  await em.save(Expense, expenseData.map(d => em.create(Expense, d)));
  ...
  await em.save(Treasury, treasuryData.map(d => em.create(Treasury, d)));
  ...
  await em.save(JournalEntry, journalData.map(d => em.create(JournalEntry, d)));
  ...
  await em.save(Document, documentData.map(d => em.create(Document, d)));
