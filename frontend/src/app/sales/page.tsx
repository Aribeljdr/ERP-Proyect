...
export default function SalesPage() {
  ...
  const confirmDelete = async () => {
    ...
    await fetch(`http://localhost:4000/api/v1/sales/${deleteItem.id}`, { method: 'DELETE' })
    setDeleteItem(null)
    setRefreshKey(k => k + 1)
  }
  ...
  return (
    <div style={{ maxWidth: '1360px', margin: '0 auto', animation: 'fadeIn .3s ease' }}>
      <div className="flex items-center justify-between mb-[18px] flex-wrap gap-3">
        <div className="text-[20px] font-bold tracking-tight" style={{ color: 'var(--text)' }}>Órdenes de Venta</div>
