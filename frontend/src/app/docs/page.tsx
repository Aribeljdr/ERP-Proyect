...
import { DocModal } from '@/components/docs/doc-modal'
...
export default function DocsPage() {
  ...
  const handleEdit = (d: any) => { setEditItem(d); setModalOpen(true) }
  ...
  return (
    <div style={{ maxWidth: '1360px', margin: '0 auto', animation: 'fadeIn .3s ease' }}>
      <div className="flex items-center justify-between mb-[18px] flex-wrap gap-3">
        <div className="text-[20px] font-bold tracking-tight" style={{ color: 'var(--text)' }}>Documentos</div>
        <div className="flex gap-2.5">
          <button className="btn-ghost h-10"><Filter className="w-[15px] h-[15px]" />Filtros</button>
          <button className="btn-primary h-10" onClick={handleNew}><Plus className="w-4 h-4" strokeWidth={2.2} />Subir documento</button>
        </div>
      </div>
