...
const CATEGORIES = ['Ventas', 'Servicios', 'Inversiones', 'Salarios', 'Operativos', 'Marketing', 'Impuestos', 'Otros']
...
export function TreasuryModal({ open, onClose, onSave, item }: TreasuryModalProps) {
  const [form, setForm] = useState({ concept: '', type: 'income', amount: '', date: '', category: '', status: 'completed' })
  ...
  useEffect(() => {
    if (item) {
      ...
      setForm({ concept: '', type: 'income', amount: '', date: '', category: '', status: 'completed' })
    }
  }, [item, open])
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(0,0,0,.35)', backdropFilter: 'blur(2px)' }}>
      <div className="w-full max-w-md rounded-2xl p-0 overflow-hidden" style={{ background: 'var(--card,#fff)', border: '1px solid var(--border,#E2E8F0)', boxShadow: '0 25px 60px rgba(0,0,0,.15)' }}>
        <div className="flex items-center justify-between px-6 py-4 border-b" style={{ borderColor: 'var(--border,#E8EDF3)' }}>
          <div className="text-[16px] font-bold" style={{ color: 'var(--text)' }}>{item ? 'Editar movimiento' : 'Nuevo movimiento'}</div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            <label className="block text-[12.5px] font-semibold mb-1.5" style={{ color: 'var(--text)' }}>Concepto</label>
            <input value={form.concept} onChange={e => set('concept', e.target.value)} required
