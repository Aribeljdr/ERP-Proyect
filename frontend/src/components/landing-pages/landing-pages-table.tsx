...
const STATUS_MAP: Record<string, { label: string; color: string; bg: string }> = {
  published: { label: 'Publicada', color: '#22C55E', bg: 'rgba(34,197,94,.12)' },
  draft: { label: 'Borrador', color: '#94A3B8', bg: 'rgba(148,163,184,.14)' },
  archived: { label: 'Archivada', color: '#64748B', bg: 'rgba(100,116,139,.12)' },
}

export function LandingPagesTable({ onEdit, onDelete }: { onEdit: (e: any) => void; onDelete: (e: any) => void }) {
            ...
            {loading ? (
              ...
              return (
                <tr key={p.id} className="hover:bg-hover transition-all" style={{ borderTop: '1px solid var(--border,#E8EDF3)' }}>
                  <td className="px-4 py-3.5 text-[13.5px] font-semibold" style={{ color: 'var(--text)' }}>{p.title}</td>
                  <td className="px-4 py-3.5 text-[13px] font-mono" style={{ color: 'var(--text)' }}>/{p.slug}</td>
                  ...
                  <td className="px-4 py-3.5 text-[13px]" style={{ color: 'var(--text)' }}>{p.viewsCount.toLocaleString()}</td>
                  <td className="px-4 py-3.5 text-[13px]" style={{ color: 'var(--text)' }}>{p.conversionRate}%</td>
