...
export function PurchasesModal({ open, onClose, onSave, item }: PurchasesModalProps) {
  const [form, setForm] = useState({ orderNumber: '', supplierName: '', total: '', status: 'pending', itemsCount: '' })
  ...
  useEffect(() => {
    if (item) {
      ...
      setForm({ orderNumber: '', supplierName: '', total: '', status: 'pending', itemsCount: '' })
    }
  }, [item, open])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!form.orderNumber || !form.supplierName || !form.total) return
    setSaving(true)
    const payload = { ...form, total: parseFloat(form.total), itemsCount: parseInt(form.itemsCount) || 0 }
    try {
      if (item) {
        await api.purchases.update(item.id, payload)
      } else {
      ...
      <div className="w-full max-w-md rounded-2xl p-0 overflow-hidden" style={{ background: 'var(--card,#fff)', border: '1px solid var(--border,#E2E8F0)', boxShadow: '0 25px 60px rgba(0,0,0,.15)' }}>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[12.5px] font-semibold mb-1.5" style={{ color: 'var(--text)' }}>NÂ° Orden</label>
          ...
          <div>
            <label className="block text-[12.5px] font-semibold mb-1.5" style={{ color: 'var(--text)' }}>Proveedor</label>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={saving}
              ...
              {saving ? 'Guardandoâ€¦' : item ? 'Actualizar' : 'Crear orden'}
            </button>
          </div>
        </form>
      </div>
