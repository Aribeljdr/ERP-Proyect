...
export function PurchasesTable({ onEdit, onDelete }: { onEdit: (e: any) => void; onDelete: (e: any) => void }) {
      ...
      <div className="overflow-x-auto">
        <table className="w-full" style={{ borderCollapse: 'collapse', minWidth: '820px' }}>
          <thead><tr style={{ background: 'var(--bg,#F8FAFC)' }}>
            ...
            <th className="text-left px-4 py-3 text-[11.5px] font-semibold tracking-wider uppercase text-muted">Proveedor</th>
            ...
            {loading ? (
              ...
              return (
                <tr key={o.id} className="hover:bg-hover transition-all" style={{ borderTop: '1px solid var(--border,#E8EDF3)' }}>
                  ...
                  <td className="px-4 py-3.5 text-[13px]" style={{ color: 'var(--text)' }}>{o.supplierName}</td>
                  ...
                  <td className="px-4 py-3.5 text-[13px] text-muted">{o.itemsCount}</td>
