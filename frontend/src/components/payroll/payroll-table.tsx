...
export function PayrollTable({ onEdit, onDelete }: { onEdit: (e: any) => void; onDelete: (e: any) => void }) {
    ...
    <div style={{ background: 'var(--card,#fff)', border: '1px solid var(--border,#E2E8F0)', borderRadius: '18px', boxShadow: 'var(--elev)', overflow: 'hidden' }}>
      {loading ? (
        ...
        <div className="grid grid-cols-4 gap-4 p-5 border-b" style={{ borderColor: 'var(--border)' }}>
          ...
          <div><div className="text-[13px] font-medium text-muted">Pagados</div><div className="text-[24px] font-bold text-green-500">{summary.paidRecords}</div></div>
          <div><div className="text-[13px] font-medium text-muted">Pendientes</div><div className="text-[24px] font-bold text-amber-500">{summary.pendingRecords}</div></div>
          <div><div className="text-[13px] font-medium text-muted">Total pagado</div><div className="text-[24px] font-bold" style={{ color: 'var(--text)' }}>${summary.totalPaid.toLocaleString()}</div></div>
        </div>
      )}
      <div className="overflow-x-auto">
        <table className="w-full" style={{ borderCollapse: 'collapse', minWidth: '820px' }}>
          <thead><tr style={{ background: 'var(--bg,#F8FAFC)' }}>
            ...
            <th className="text-left px-4 py-3 text-[11.5px] font-semibold tracking-wider uppercase text-muted">Periodo</th>
            <th className="text-left px-4 py-3 text-[11.5px] font-semibold tracking-wider uppercase text-muted">Bruto</th>
            <th className="text-left px-4 py-3 text-[11.5px] font-semibold tracking-wider uppercase text-muted">Deducciones</th>
            <th className="text-left px-4 py-3 text-[11.5px] font-semibold tracking-wider uppercase text-muted">Neto</th>
            ...
            {loading ? (
              ...
              return (
                <tr key={r.id} className="hover:bg-hover transition-all" style={{ borderTop: '1px solid var(--border,#E8EDF3)' }}>
                  ...
                  <td className="px-4 py-3.5 text-[13px]" style={{ color: 'var(--text)' }}>{r.period}</td>
                  <td className="px-4 py-3.5 text-[13px]" style={{ color: 'var(--text)' }}>${Number(r.grossSalary).toLocaleString()}</td>
                  <td className="px-4 py-3.5 text-[13px]" style={{ color: '#EF4444' }}>-${Number(r.deductions).toLocaleString()}</td>
                  <td className="px-4 py-3.5 text-[13.5px] font-semibold" style={{ color: 'var(--text)' }}>${Number(r.netPay).toLocaleString()}</td>
